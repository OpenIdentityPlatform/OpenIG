...
xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] describes how to set up OpenIG as a SAML 2.0 service provider for a single application, using OpenAM as the identity provider. This chapter describes how to set up OpenIG as a SAML 2.0 service provider for two applications, still using OpenAM as the identity provider.
Before you try the samples described here, familiarize yourself with OpenIG SAML 2.0 support by reading and working through the examples in xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] . Before you start, you should have OpenIG protecting the sample application as a SAML 2.0 service provider, with OpenAM working as identity provider configured as described in that tutorial.

[#federation-installation-multi]
=== A.1. Installation Overview

In this chapter you use the Fedlet configuration from xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] to create a configuration for each new protected application. You then import the new configurations as SAML 2.0 entities in OpenAM. If you subsequently edit a configuration, import it again.
...
a| xref:#prepare-network-multi[Section A.2, "Preparing the Network"] 


a|Prepare the configuration for two OpenIG service providers.

a| xref:#prepare-cot-multi[Section A.3, "Configuring the Circle of Trust"] 
xref:#prepare-saml-conf1-multi[Section A.4, "Configuring the Service Provider for Application One"] 
xref:#prepare-saml-conf2-multi[Section A.5, "Configuring the Service Provider for Application Two"] 


a|Import the service provider configurations into OpenAM.

a| xref:#multisp-create-saml-entities[Section A.6, "Importing Service Provider Configurations Into OpenAM"] 


a|Add OpenIG routes.

a| xref:#multisp-conf[Section A.7.1, "Preparing the Base Configuration File"] 
xref:#multisp-conf-sp1[Section A.7.2, "Preparing Routes for Application One"] 
xref:#multisp-conf-sp2[Section A.7.3, "Preparing Routes for Application Two"] 
...
Edit the `$HOME/.openig/SAML/fedlet.cot` file you created in xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] to include the entity IDs `sp1` and `sp2` , as in the following example:
...
To configure the service provider for application one, you can use the example files xref:#multisp-sp1[Example A.1, "Configuration File for Application One"] and xref:#multisp-sp1-extended[Example A.2, "Extended Configuration File for Application One"] , saving them as `sp1.xml` and `sp1-extended.xml` . Alternatively, follow the steps below to use the files you created in xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] .

[#d2528e8994]
.Procedure A.1. To Configure the Service Provider for Application One By Using Files Created In Chapter 7, "OpenIG As a SAML 2.0 Service Provider"
====

. Copy the SAML configuration files `sp.xml` and `sp-extended.xml` you created in xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] , and save them as `$HOME/.openig/SAML/sp1.xml` and `$HOME/.openig/SAML/sp1-extended.xml` .
...
+
For an example of how this file should be, see xref:#multisp-sp1[Example A.1, "Configuration File for Application One"] .
...
+
For an example of how this file should be, see xref:#multisp-sp1-extended[Example A.2, "Extended Configuration File for Application One"] .
...
. Copy the SAML configuration files `sp1.xml` and `sp1-extended.xml` you created in xref:#prepare-saml-conf1-multi[Section A.4, "Configuring the Service Provider for Application One"] , and save them as `$HOME/.openig/SAML/sp2.xml` and `$HOME/.openig/SAML/sp2-extended.xml` .
...
For each new protected application, prepare an OpenIG configuration. The configurations in this section follow the example in xref:chap-federation.adoc#chap-federation[Chapter 7, "OpenIG As a SAML 2.0 Service Provider"] .
