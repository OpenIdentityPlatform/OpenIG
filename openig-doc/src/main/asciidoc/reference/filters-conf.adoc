...
and body against the specification. If validation fails, the filter returns a
`400 Bad Request` response immediately. The downstream handler is not called.
The response body is `text/plain` and contains one validation message per line.
...
* `true` — the filter returns a `502 Bad Gateway` response containing the
validation messages.
* `false` (default) — the violation is logged at `WARN` level and the original
