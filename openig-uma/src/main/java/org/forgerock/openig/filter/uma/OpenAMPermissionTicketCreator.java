...
public class OpenAMPermissionTicketCreator implements PermissionTicketCreator {
    ...
    private final URI permissionRequestEndpoint;
    ...
    public OpenAMPermissionTicketCreator(Handler client,
                                         URI permissionRequestEndpoint,
                                         DemoPatGetter patGetter,
                                         Timer timer) {
        this.client = client;
    ...
    public String create(String resourceSetId, Collection<String> requiredScopes) throws OAuth2TokenException {
        ...
        request.setUri(permissionRequestEndpoint);
        request.setEntity(json(object(field("resource_set_id", resourceSetId),
                                      field("scopes", requiredScopes)))
                                  .asMap());
        ...
        if (isResponseEmpty(response)) {
            throw new OAuth2TokenException("Authorization Server did not return any ticket");
        }
